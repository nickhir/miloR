...
$$L(p; r, y) = \mbox{exp}\bigg( \underbrace{ylog[1-p]}_{t(y)\eta} - \overbrace{rlog[1-p] + rlogp}^{b(\eta)} + \underbrace{log\big[\frac{\Gamma(y)}{\Gamma(r)}\big]}_{c(y, \phi)} \bigg)$$

From this we know that $\theta=(1-p)$ and $\eta=log[1-p]$. Therefore $1-p=e^{\eta}$ and $p=e^{\eta}+1$. What we don't know is the value for $a(\phi)$.  Importantly, using this 
...
$Var[Y]=\phi \frac{d^2b(\theta)}{d\eta^2}$.

### Finding the mean and variance

$$b(\eta) = -rlog[e^\eta] + rlog[e^\eta+1]$$
Let $u=e^\eta$, $v=logu$ $a=e^n+1$ and $b=rloga$ then using the sum rule and the chain rule:

$$E[Y]=\mu=\frac{db(\eta)}{d\eta}=(\frac{du}{d\eta}) (\frac{dv}{du}) + (\frac{da}{d\eta}) (\frac{db}{da})= [e^\eta \cdot (\frac{-r}{e^n})] + [e^\eta \cdot {\frac{r}{e^\eta +1}}] = -r + \frac{re^\eta}{e^\eta+1}$$

Substituting $e^\eta+1 = p$ and $e^\eta = 1-p$

$$E[Y]=\mu=\frac{db(\eta)}{d\eta}=-r+(1-p) \cdot r(p)^{-1} = \frac{r(1-p)}{p}-r$$

Expanding out the brackets and simplifying we end up with:

$$E[Y]=\mu=\frac{db(\eta)}{d\eta}=\frac{r-pr}{p} - r = -r + \frac{r}{p} - \frac{pr}{p} = -r + \frac{r}{p} - r = \frac{r}{p}$$

Thus $E[Y]=\mu=\frac{r}{p}$. Using this we can also find the variance ($Var[Y]$) and the value for $\phi$ as follows (using the identity $p=e^\eta+1$):

$$Var[Y]=\phi\frac{d^2b(\eta)}{d\eta^2} = \phi\frac{d(db(\eta)/d\eta)}{d\eta} = \phi\frac{d\mu}{d\eta}=\phi\frac{d\frac{r}{p}}{\eta}=\phi\frac{d\frac{r}{e^\eta+1}}{d\eta}$$

As we have a quotient, we can use the quoitent rule to find this derivative, where the quotient rule is:

$$\frac{df}{dt}= \frac{v\frac{du}{dt} - u\frac{dv}{dt}}{v^2}$$

Therefore, let $u=re^\eta$ and $v=e^\eta+1$, with $\frac{du}{d\eta} = re^\eta$ and $\frac{dv}{d\eta}=e^\eta$ to get:

$$Var[Y]=\phi\frac{d\frac{r}{e^\eta+1}}{d\eta} = \frac{(e^\eta+1)re^\eta - (re^\eta \cdot e^\eta)}{(e^\eta+1)^2} $$

Expanding brackets and simplifying:

$$Var[Y]=\phi\frac{d\frac{r}{e^\eta+1}}{d\eta} = \frac{r(e^\eta)^2 + re^\eta - r(e^\eta)^2}{(e^\eta+1)^2} = \frac{re^\eta}{(e^\eta+1)^2} $$

Replacing $e^\eta=1-p$ and $e^\eta+1=p$:

$$Var[Y]=\phi\frac{d\frac{r}{e^\eta+1}}{d\eta} = \frac{r(1-p)}{p^2} $$
...
$$M''(t) = r(pe^t)^r(-r-1)[1-(1-p)e^t]^{-r-2}[-(1-p)e^t]+r^2(pe^t)^{r-1}(pe^t)^{r-1}pe^t[1-(1-p)e^t]^{-r-1}$$
...
$$E[Y^2] = M''(0) = rp^r(-r-1)p^{-r-2}[-(1-p)]+r^2p^{r-1}p^{r-1}pp^{-r-1} \\ = rp^r(-1)(r+1)[1-(1-p)]^{-r-2}(-1)(1-p)+r^2p^{r-1}p[1-(1-p)]^{-r-1}$$
...
Now we can compute $Var[Y] = E[Y^2] - E[Y]^2$ and equate it with our variance definition above $\phi\frac{d\frac{r}{e^\eta+1}}{d\eta} = \frac{r(1-p)}{p^2}$.
...
$$\theta = 1 - \frac{r}{\mu} \\ b(\eta) = -rlog\bigg[1 - \frac{r}{\mu} \bigg] + rlogr + rlog\mu \\ t(y) = y \\ \eta = log[1- \frac{r}{\mu}]  \\ c(y) = log \bigg[ \frac{\Gamma(y)}{\Gamma(r)} \bigg] \\ e^\eta = 1 - \frac{r}{\mu} \\ e^\eta + 1 = \frac{r}{\mu} \\ \mu=\frac{r}{e^\eta +1}$$

and in exponential family form:

$$ f(y, \theta, \phi) =exp \bigg\{ \frac{\overbrace{y}^{t(y)} \overbrace{log \bigg[1-\frac{r}{\mu} \bigg]}^{\theta} - \overbrace{rlog\bigg[1 - \frac{r}{\mu} \bigg] + rlogr + rlog\mu }^{b(\theta)} }{\phi} + \overbrace{log \bigg[\frac{\Gamma(y)}{\Gamma(r)} \bigg]}^{c(y, \phi)} \bigg\}$$
...
$$f(y, b, \mu) = \int NegativeBinomial(\frac{r}{\mu}, r) \ Normal(0, \sigma^2) \ \mathbf {db}\\ log \ f(y, b, \mu) =\int ylog \bigg[1-\frac{r}{\mu} \bigg] - rlog\bigg[1 - \frac{r}{\mu} \bigg] + rlogr + rlog\mu + log \bigg[\frac{\Gamma(y)}{\Gamma(r)} \bigg] -(\frac{1}{2})log2\pi - \frac{1}{2}log (2\sigma^2) - \frac{1}{2}(\frac{b}{\sigma})^2 \ db$$

This is a pretty horrible integral, especially because $\mu=exp(\eta) = exp(X\beta + Zb)$, and for $\gt1$ random effect this becomes a high dimensional integral - nasty. 
...
## Variance partial derivatives

For the marginal quasi-likelihood (and possibly the full loglihood approach?), we need the partial derivatives of the total variance w.r.t. each of the 
variance components. The first partial derivatives are shown below:

$$V = V_{0} + exp(ZGZ^T) \\  V_{0}= \underset{n \times n}{diag} \frac{\mu_i^2}{r} \mu_i \\ \frac{\partial{V}}{\partial \sigma_j} = \Lambda + 2\sigma_j exp(ZGZ^T)A \\ \Lambda = \underset{n \times n}{diag} \ CDZs^T \\ A = Z_{\cdot j} Z^T_{j \cdot} \\ C = \underset{n \times n}{diag} \frac{2\mu_i}{r} - 1 \\ D = \underset{n \times n}{diag} \ \mu_i \\ s = [0 \cdots \frac{\partial b_j}{\partial \sigma_j} \cdots 0] \\ \frac{\partial u_j}{\partial \sigma_j} = \bigg(\frac{1}{q-1} \sum_{l=1}^q 2u_{lj}  \bigg)^{-1}$$

We also need the elements of the Hessian $H(\sigma)$ that include the second partial derivatives:

$$ \frac{\partial^2 V}{\partial \sigma_i \partial \sigma_j} = PQ +MN + O + 2 exp(ZGZ^T)A[I + 2\sigma_j^2 A ] \\ P = \underset{n \times n}{diag} \ RDZs^T \\ R = \frac{2}{r} I_{n\times n} \\ Q=\underset{n \times n}{diag} \ DZs^T \\ M = \underset{n \times n}{diag} \ CDZs^T \\ N = \underset{n \times n}{diag} \ Zs^T \\ O = \underset{n \times n}{diag} \ CDZw^T \\ w = [0 \cdots \frac{q-1}{2q} \cdots 0] $$

Because I have ignored the interaction terms (random slopes), when $i \neq j$ then these elements are 0.
